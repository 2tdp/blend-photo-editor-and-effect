...
public class HeadController {
    ...
    protected PointF mDestPt1 = null, mDestPt2 = null, mDestPt3 = null, mDestPt4 = null;
    private PointF mCenterPT = null;
    private PointF mOriginalDestPt1 = null, mOriginalDestPt2 = null, mOriginalDestPt3 = null, mOriginalDestPt4 = null;
    ...
    public HeadController(Context context, Rect srcroi, HeadInfo imgData, int boundaryType, boolean freeRect) {
        ...
        if (srcroi.left < 0) srcroi.left = 0;
        if (srcroi.top < 0) srcroi.top = 0;
        ...
        mbApply = (srcroi.left <= srcroi.right) && (srcroi.top <= srcroi.bottom);
    ...
    protected void setRectRoi(Rect roi) {
        ...
        if (rectWidth > rectHeight) {
            ...
            int absoluteMarginX = (int) ((mImageData.getDrawCanvasRoi().width() - mROI.width()) / 2);
            ...
            int absoluteMarginY = (int) ((mImageData.getPreviewHeight() - mROI.height()) / 3);
    ...
    public void destroy() {
        ...
        if (mBitmapResource != null) mBitmapResource.recycle();

        mBitmapResource = null;
        if (mBitmapRotateResource != null) mBitmapRotateResource.recycle();
    ...
    protected RectF getDrawROI() {
        if (mDrawROI != null) {
            Matrix m = new Matrix();
            m.postTranslate(mImageData.getDrawCanvasRoi().left, mImageData.getDrawCanvasRoi().top);
            RectF src = new RectF();
            src.set(mROI);
            RectF dst = new RectF();
            m.mapRect(dst, src);
    ...
    public void setOrgDestROI(RectF roi, PointF center, PointF mOrgDestPt1,
        ...
        if (mImageData != null) {
            ...
            m.getValues(values);
            if (values[Matrix.MTRANS_X] > 0) left = 0;
            else left = values[Matrix.MTRANS_X];
            if (values[Matrix.MTRANS_Y] > 0) top = 0;
            else top = values[Matrix.MTRANS_Y];
            ...
            mROI.set((int) dst.left, (int) dst.top, (int) dst.right, (int) dst.bottom);
    ...
    protected int InitMoveObject(float x, float y) {
        ...
        if (mROI.width() < AVAILABLE_TOUCH_AREA_OFFSET * 4 || mROI.height() < AVAILABLE_TOUCH_AREA_OFFSET * 4) {
            if (mROI.width() > mROI.height()) real_touch_area_offset = (int) (mROI.height() / 4);
            else real_touch_area_offset = (int) (mROI.width() / 4);
            if (real_touch_area_offset < 1) real_touch_area_offset = 1;
        }

        float h_mid;
        float v_mid;

        if (Math.abs(x - mDestPt2.x) < real_touch_area_offset && Math.abs(y - mDestPt2.y) < real_touch_area_offset) {
            ...
            mTouchType = RIGHT;
                ...
                && mFreeRect)
            mTouchType = BOTTOM;
        else if (Math.abs(x - mDestPt1.x) < real_touch_area_offset && Math.abs(y - mDestPt1.y) < real_touch_area_offset)
            mTouchType = LEFT_TOP;
        else if (Math.abs(x - mDestPt3.x) < real_touch_area_offset && Math.abs(y - mDestPt3.y) < real_touch_area_offset)
            mTouchType = RIGHT_BOTTOM;
        else if (Math.abs(x - mDestPt4.x) < real_touch_area_offset && Math.abs(y - mDestPt4.y) < real_touch_area_offset)
            mTouchType = LEFT_BOTTOM;
        else if (checkPointInRect(x, y, mDestPt1, mDestPt2, mDestPt3, mDestPt4)) mTouchType = MOVE;
    ...
    protected void StartMoveObject(float x, float y) {
        ...
        if (mTouchType == RIGHT_TOP) {
            if (mRightTopRotate) {
                ...
                PointF pt;
                pt = getRotationPoint(mROI.left, mROI.top, angle, mCenterPT.x, mCenterPT.y);
                mDestPt1.set(pt.x, pt.y);
                pt = null;

                pt = getRotationPoint(mROI.right, mROI.top, angle, mCenterPT.x, mCenterPT.y);
                mDestPt2.set(pt.x, pt.y);
                pt = null;

                pt = getRotationPoint(mROI.right, mROI.bottom, angle, mCenterPT.x, mCenterPT.y);
                mDestPt3.set(pt.x, pt.y);
                pt = null;

                pt = getRotationPoint(mROI.left, mROI.bottom, angle, mCenterPT.x, mCenterPT.y);
                mDestPt4.set(pt.x, pt.y);
                pt = null;

                if (mBoundaryType == RECT_CENTER_BOUNDARY) mAngle = angle;
            } else resizeRectWithRotationAngle(x, y, width, height, mTouchType);
            ...
            if (mTouchType == MOVE) {
                ...
                objectROI.left = Math.min(mDestPt4.x, Math.min(mDestPt3.x, Math.min(mDestPt1.x, mDestPt2.x)));
                objectROI.right = Math.max(mDestPt4.x, Math.max(mDestPt3.x, Math.max(mDestPt1.x, mDestPt2.x)));
                objectROI.top = Math.min(mDestPt4.y, Math.min(mDestPt3.y, Math.min(mDestPt1.y, mDestPt2.y)));
                objectROI.bottom = Math.max(mDestPt4.y, Math.max(mDestPt3.y, Math.max(mDestPt1.y, mDestPt2.y)));

                mCenterPT.x = (objectROI.left + objectROI.right) / 2;
                mCenterPT.y = (objectROI.top + objectROI.bottom) / 2;

                PointF pt;
                pt = getRotationPoint(mDestPt1, -mAngle, mCenterPT.x, mCenterPT.y);
                ...
                pt = null;
            } else if (mTouchType <= LEFT_BOTTOM) resizeRectWithRotationAngle(x, y, width, height, mTouchType);
        }
...
//		calculateOriginal();
    ...
    private void resizeRectWithRotationAngle(float x, float y, int width, int height, int mode) {

        PointF CurrPt1 = getRotationPoint(x, y, -mAngle, mCenterPT.x, mCenterPT.y);
        PointF CurrPt2 = getRotationPoint(mPrevX, mPrevY, -mAngle, mCenterPT.x, mCenterPT.y);
        ...
        else if (mode == LEFT_TOP || mode == RIGHT_TOP || mode == LEFT_BOTTOM || mode == RIGHT_BOTTOM) {
            float[] diff = getDiffFromCropRatio(dx, dy, mode);
            tempRoi = getClipROI(mROI, mode, diff[0], diff[1], 0, 0, width, height);
        }
    ...
    public void EndMoveObject() {
        if (mTouchType == RIGHT_TOP || mTouchType == MOVE) {
            ...
            if (mBoundaryType == RECT_CENTER_BOUNDARY) {
                PointF pt;
                pt = getRotationPoint(mROI.left, mROI.top, mAngle, mCenterPT.x, mCenterPT.y);
                mDestPt1.set(pt.x, pt.y);
                pt = null;

                pt = getRotationPoint(mROI.right, mROI.top, mAngle, mCenterPT.x, mCenterPT.y);
                mDestPt2.set(pt.x, pt.y);
                pt = null;

                pt = getRotationPoint(mROI.right, mROI.bottom, mAngle, mCenterPT.x, mCenterPT.y);
                mDestPt3.set(pt.x, pt.y);
                pt = null;

                pt = getRotationPoint(mROI.left, mROI.bottom, mAngle, mCenterPT.x, mCenterPT.y);
                mDestPt4.set(pt.x, pt.y);
                pt = null;
            }
...
//		calculateOriginal();
    ...
    public void setDestROI(float left, float top, float right, float bottom) {
        ...
        pt = getRotationPoint(mROI.right, mROI.bottom, mAngle, offset_x, offset_y);
        mDestPt3.set(pt.x, pt.y);
        pt = null;

        pt = getRotationPoint(mROI.left, mROI.bottom, mAngle, offset_x, offset_y);
        mDestPt4.set(pt.x, pt.y);
        pt = null;
    ...
    private void initResource(Context context, HeadInfo imgInfo) {
        ...
        mDrawLinePaint.setPathEffect(new DashPathEffect(new float[]{5, 5}, 3));
    ...
    static public PointF getRotationPoint(PointF srcPoint, int angle, float pivotX, float pivotY) {
    ...
    static public PointF getRotationPoint(float srcX, float srcY, float angle, float pivotX, float pivotY) {
        PointF retPt = new PointF();
        float radian = (float) Math.toRadians(angle);
        double cos = Math.cos(radian);
        double sin = Math.sin(radian);

        retPt.x = (int) Math.round(((srcX - pivotX) * cos - (srcY - pivotY) * sin));
        retPt.y = (int) Math.round(((srcX - pivotX) * sin + (srcY - pivotY) * cos));
    ...
    static public boolean checkPointInRect(float x, float y, PointF P1, PointF P2, PointF P3, PointF P4) {
        PointF currentPoint = new PointF();
        currentPoint.set(x, y);

        if (ccw(P1, P2, currentPoint) > 0
                && ccw(P2, P3, currentPoint) > 0
