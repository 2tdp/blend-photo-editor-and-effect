...
object UtilsBitmap {
    ...
    fun resizeBitmap(bitmap: Bitmap, width: Int, height: Int): Bitmap {
        val img = Bitmap.createScaledBitmap(bitmap, width, height, true)
        bitmap.recycle()
        return img
    }
    ...
    fun createByteImage(context: Context, uriPic: String?, callBack: ICallBackItem) {
        val bmp: Bitmap
        try {
            bmp = getBitmapFromUri(context,
                Uri.parse(uriPic))?.let { modifyOrientation(context, it, Uri.parse(uriPic)) }!!
