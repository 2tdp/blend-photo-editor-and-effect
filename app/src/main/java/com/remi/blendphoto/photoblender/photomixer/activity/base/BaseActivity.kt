package com.remi.blendphoto.photoblender.photomixer.activity.base
...
abstract class BaseActivity : AppCompatActivity() {
    ...
    protected fun replaceFragmentBottom(manager: FragmentManager, fragment: Fragment, isAdd: Boolean, addBackStack: Boolean, isAnimation: Boolean) {
        try {
            val fragmentTransaction = manager.beginTransaction()
            if (isAnimation) fragmentTransaction.setCustomAnimations(enterBottom, exitBottom, popEnterBottom, popExitBottom)
            if (addBackStack) fragmentTransaction.addToBackStack(fragment.javaClass.simpleName)
            if (isAdd) fragmentTransaction.add(res, fragment, fragment.javaClass.simpleName)
            else fragmentTransaction.replace(res, fragment, fragment.javaClass.simpleName)
